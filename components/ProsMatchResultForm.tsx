...
export default function ProsMatchResultForm({ matchId }: { matchId: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error: updateError } = await supabase
      .from('pronos_matches')
      .update({ team_a_score: Number(scoreA), team_b_score: Number(scoreB), status: 'finished' })
      .eq('id', matchId);

    setSaving(false);
    if (updateError) {
      setError(
        updateError.message.includes('policy') ? 'Seuls les admins peuvent saisir un résultat.' : updateError.message
      );
      return;
    }
    setOpen(false);
